...
bool LeftMFD::OnLoadPanel2D(int id, PANELHANDLE hPanel)
{
	...
	vcFont_.blankY = 2;
														...
														// Mesh coords
	int rc_left = 150;									// Left side of left col.
	int rc_col2_offset = 690 - rc_left;					// Diff between right col and left
...
//		oapiRegisterPanelArea(a.Id, _R(rc_l, rc_t, rc_r, rc_b), PANEL_REDRAW_USER);
	...
	vessel->RegisterPanelArea(
		hPanel,
		GetPwrKey(),
		_R(245, fixedYTop, 245 + btn_width, fixedYTop + btn_height),
		PANEL_REDRAW_NEVER, 
	...
	vessel->RegisterPanelArea(
		hPanel,
		GetSelectKey(),
		_R(518, fixedYTop, 518 + btn_width, fixedYTop + btn_height),
		PANEL_REDRAW_NEVER,
		PANEL_MOUSE_LBDOWN | PANEL_MOUSE_ONREPLAY);
	...
	vessel->RegisterPanelArea(
		hPanel,
		GetSelectKey(),
		_R(595, fixedYTop, 595 + btn_width, fixedYTop + btn_height),
		PANEL_REDRAW_NEVER,
		PANEL_MOUSE_LBDOWN | PANEL_MOUSE_ONREPLAY);
...
bool LeftMFD::OnPanelMouseEvent(int id, int event)
{
	return OnMouseEvent(id, event);
}
