...
class AnimationGroup {
 public:
  AnimationGroup(std::initializer_list<UINT> const& grp, const VECTOR3& locA, const VECTOR3& locB, double angle, double start, double stop) :
      group_(grp),
      start_(start),
      stop_(stop),
      location_(locA) {
...
*/
using animation_target = animation_base<StateUpdateTarget>;

// animation_target with 'wrap' target update.
using animation_wrap = animation_base<StateUpdateWrap>;
}